...
const getParams = () => {
    ...
    const fee = toWei(faker.finance.amount(0.01, 0.05))
...
async function main() {
    ...
    const Contract = await ethers.getContractFactory('Giveaway')
    ...
    const createGiveaway = async ({ title, description, prize, fee, expiresAt, image }) => {
      tx = await contract.functions.createGiveaway(
        title,
        description,
        image,
        prize,
    ...
    for (let i = 0; i < iterations; i++) {
      await createGiveaway(getParams())
    }
  
    result = await contract.getGiveaways()
    console.log(result)
    // Seeding autogenerates ends here...
    ...
    fs.writeFile('./artifacts/contractAddress.json', address, 'utf8', (err) => {
      ...
      console.log('Giveaway Contract Deployed contract address', contract.address)
    })
  }
